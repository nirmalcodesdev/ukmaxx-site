...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  cards.forEach(card=>{
    ...
    panel.innerHTML=`<div class='tabs'><button class='tab-btn' data-tab='science'>SCIENCE</button><button class='tab-btn' data-tab='specs'>SPECS</button><button class='tab-btn active' data-tab='coa'>COA</button></div><div class='tab-pane science' data-pane='science'>${d.science}</div><div class='tab-pane' data-pane='specs'><dl class='dl'>${specsRows}</dl>${needsBac?`<div class='info-box'>Reconstitution requires bacteriostatic water. ${showBacBtn?`<button class='add-btn' data-bac style='min-height:30px;padding:4px 10px;margin-left:6px'>ADD BAC WATER</button>`:''}</div>`:''}</div><div class='tab-pane active' data-pane='coa'><dl class='dl'>${coaRows}</dl><a href='#' class='coa-trigger' style='color:var(--accent);text-decoration:none'>View Details & COA ↓</a></div>`;
